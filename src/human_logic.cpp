    ...
    for (int d1 = 1; d1 <= 9; ++d1) {
      ...
      for (int d2 = d1 + 1; d2 <= 9; ++d2) {
        if (appears[d1 - 1] != appears[d2 - 1])
          continue;
        const uint16_t keep =
            static_cast<uint16_t>(DigitBit(d1) | DigitBit(d2));
        for (size_t i = 0; i < unit.size(); ++i) {
          if (!(appears[d1 - 1] & (1u << i)))
            continue;
          const int row = unit[i] / 9, col = unit[i] % 9;
          if (state.board[unit[i]] != 0)
            continue;
          const uint16_t mask = state.candidates(row, col);
          const uint16_t remaining = static_cast<uint16_t>(mask & keep);
          if (Popcount(remaining) == 1 &&
              state.place(row, col, BitToDigit(remaining))) {
