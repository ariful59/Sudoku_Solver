...
struct DfsSolver {
  ...
  explicit DfsSolver(const Board &input) : state(input) {}
...
SolveResult SolveWithConfig(const Board &input, int solutionLimit,
...
SolveResult SolveSudokuUnique(const Board &input, int solutionLimit,
                              bool useHumanTechniques) {
  return internal::SolveWithConfig(input, solutionLimit, useHumanTechniques);
}

bool GenerateSolvedBoard(Board &outBoard, bool useHumanTechniques) {
  Board empty{};
  const SolveResult result =
      internal::SolveWithConfig(empty, 1, useHumanTechniques);
